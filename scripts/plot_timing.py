...
print(f"Found delta of {delta}s, this corresponds to {100.0*delta/algorithm_timings[0]}% of algorithm timing")
finding_subtimings[0] += 0.5*delta
finding_subtimings[1] += 0.5*delta

xs = np.cumsum([0.0,] + algorithm_timings[:-1])
print(xs)
print(algorithm_timings)

fig, ax = plt.subplots(figsize=(8,4))
items1 = ax.barh(0.5, width=algorithm_timings, left=xs, 
                 color=["tab:blue","tab:orange","tab:green"])

xs2 = np.cumsum([0.0, *finding_subtimings[:-1]])

items2 = ax.barh(-0.5, width=finding_subtimings, left=xs2, alpha=1.0,
...
leg1 = ax.legend(items1, ["Track finding", "Parameter Estimation", "Kalman Filter"], loc=(0.6,0.05), title="Full chain")
